  ...
  methods: {
    async selectEmotion(id) {
      try {
        ...
        const note = '' // <--- добавь эту строку!
        console.log('Сохраняю эмоцию:', { telegramId, emotion: id, note, timestamp: new Date().toISOString() });
        await dbService.saveEmotion(telegramId, {
          emotion: id,
          note,
