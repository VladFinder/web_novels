...
API_URL="${API_URL:-https://iki.commandc.ru/api}"

echo "=== Тестирование API через PHP прокси (${API_URL}) ==="

# Тест 1: GET запрос
echo "1. Тестируем GET запрос..."
curl -s -X GET "${API_URL}/emotions/test_user/2025-06-22"
echo -e "\n"

# Тест 2: POST запрос для эмоции
echo "2. Тестируем POST запрос для эмоции..."
curl -s -X POST "${API_URL}/emotions" \
...
curl -s -X POST "${API_URL}/thoughts" \
  -H "Content-Type: application/json" \
...
curl -s -X GET "${API_URL}/emotions/stats/test_user_$(date +%s)?startDate=2025-06-01&endDate=2025-06-30"
echo -e "\n"
